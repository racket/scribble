...
;; make-hyperref-ref : number? content? -> element?
;; Return an element renders the given content
;;  and, in LaTeX documents that `\usepackage{hyperref}`, links to the given label.
(define (make-hyperref-ref i content)
  (make-element
    (make-style "Autobibhyperref" (list (command-extras (list (format-autobib-label i)))))
    (list content)))

;; make-hyperref-label : number? -> element?
;; Return an element that elements generated by `make-hyperref-ref` can link to.
(define (make-hyperref-label i)
  (make-element (make-style "label" '()) (format-autobib-label i)))

;; format-autobib-label : number? -> string?
;; Turn a number into a (hopefully unique) label for a bib entry.
(define (format-autobib-label i)
...
(define author+date-style%
  (class object%
    ...
    (define/public (render-citation date-cite i) (make-hyperref-ref i date-cite))
    (define/public (render-author+dates author dates) (list* author " " dates))
    (define/public (bibliography-line i e)
      (list (make-paragraph plain (make-hyperref-label i))
            e))
    (super-new)))
