          ...
          (let* ([strs (map (lambda (flows)
                 ...
                 [alignss
                  (cond
                    ...
                    [(ormap (lambda (v) (and (table-columns? v) v))
                            (style-properties (table-style i)))
                     =>
                     (lambda (tc)
                 ...
                 [extract-border (lambda (s)
                                   ...
                                   (cond
                                     ...
                                     [else
                                      (vector (memq 'left-border p)
                                              (memq 'right-border p)
                                              (memq 'top-border p)
                                              (memq 'bottom-border p))]))]
                 ...
                 [col-borders (for/list ([i (in-range (length (car borderss)))])
            ...
            (define (show-row-border prev-borders borders)
              (when (for/or ([prev-border (in-list prev-borders)]
                      ...
                      (or (border-bottom? prev-border) (border-top? border)))
                (define-values (end-h-border? end-v-border?)
                  (for/fold ([left-border? #f]
                    ...
                    (define border?
                      (or (and prev-border (border-bottom? prev-border)) (border-top? border)))
                     ...
                     (for/fold ([indent? indent?])
                       ...
                       (define-values (end-border? end-col-border?)
                         (for/fold ([left-border? #f]
                           ...
                           (values (border-right? border) (border-right? col-border))))
                       (when end-col-border?
